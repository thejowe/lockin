...
- [ ] **Pegar esa migración en el SQL Editor de `grrzmzktrhksbttpbblg`** — es del usuario, no hay forma de hacerlo desde el repo (solo está la clave `anon`). Mientras no se pegue, escribir un perfil contra Supabase **falla** (`Could not find the 'seeking_specialties' column…`): la suite de contrato da 27/27 fallidos y la app no puede crear ni editar perfil. Instrucciones en `supabase/README.md` → "Estado"
