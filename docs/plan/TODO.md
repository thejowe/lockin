...
- [x] **[Claude]** Hallazgo 2 — canales de vídeo y presencia sin autenticar (orden `D1`, `datos`, Ola 1). **Hecho el 2026-09-17.** `supabase/migrations/20260917000100_realtime_authorization.sql` trae las políticas de Realtime Authorization sobre `realtime.messages` (`is_session_topic_member()`, delegando en `is_match_member()`) y `video-signal.ts`/`presence.ts` pasan a `config: { private: true }`, así que el servidor las evalúa. `npm run test:schema` cubre la política contra Postgres de verdad (Ana y Bea entran, Carla y un topic ajeno reciben `42501`) y la huella ve las dos políticas nuevas. **Pendiente del usuario**: pegar la migración en el SQL Editor de `grrzmzktrhksbttpbblg` — hasta entonces el job remoto de `Schema drift` sigue rojo a propósito. Detalle en `docs/plan/todo/datos.md`
