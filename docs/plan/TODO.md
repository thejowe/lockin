...
- [ ] Sesión agendada + Pomodoro compartido + presencia + aviso local — plan `docs/superpowers/plans/2026-09-13-sesiones-lockin.md`, detalle y evidencia en `todo/sesiones.md`. Tareas 1-10 y 3b cerradas; en la Tarea 11 faltan el E2E Android del último commit en verde (ahora cae en `setup-android` por el runner) y la verificación con dos móviles. El contrato opt-in está en verde contra Supabase local en Actions (`contract.yml`, run 34897871055: 49 pasan, 0 fallan, 3 saltados a propósito por reloj simulado)
