...
- [x] **[comprobador]** Confirmar cierre y reapertura en el emulador (antes: «en el Expo Go del usuario»): APK con credenciales de Supabase (`preview` de EAS o build local con `EXPO_PUBLIC_SUPABASE_*`), perfil creado, `am force-stop` + relanzar, y el mismo perfil tiene que seguir ahí. **Sospecha del 2026-09-19:** sin credenciales la app corre contra el mock en memoria y pierde todo al cerrar — la evidencia debe decir contra qué backend corrió (`[lockin] backend de datos:` en logcat). **Verificado el 2026-09-24** contra Supabase real (`[lockin] backend de datos: Supabase` en logcat antes y después): cuenta +lockin3 / perfil «Verif», `am force-stop` → relanzado en frío → mismo perfil y misma cuenta. Detalle en `todo/arquitecto.md`.
- [x] **[Claude]** «Ya tengo cuenta» en el onboarding (`perfil`): login opcional con `signInWithEmail`/`sendPasswordReset` para recuperar la cuenta en otro dispositivo. Cerrado el 2026-09-19: `npm run typecheck`, `npm run lint` y `npm test -- --ci --runInBand` en verde, cobertura sobre el suelo. **Verificado en dispositivo el 2026-09-23** (emulador Android, APK `preview` de EAS, Supabase real): alta → correo → contraseña → perfil → `pm clear` → «Ya tengo cuenta» recupera el mismo uid y el mismo perfil. Detalle y evidencia en `todo/perfil.md`.
- [x] **[Claude]** Interfaz de registro con email y contraseña en el onboarding (`perfil`). Cerrado el 2026-09-20: **registro obligatorio** (decisión del usuario; revoca «sin login obligatorio» solo para el alta), y se **asciende la sesión anónima** (`linkEmailToCurrentUser` + `setAccountPassword` tras confirmar el correo), no `signUp`. `npm run typecheck`, `npm run lint` y `npm test -- --ci --runInBand` en verde, cobertura sobre el suelo. **Verificado en dispositivo el 2026-09-23** (correo de confirmación, cuenta recuperable, entrar tras borrar los datos de la app; ver «Verificación en dispositivo (2026-09-23)» en `todo/perfil.md`). **Cerrado por `calidad` el 2026-09-20** (`35b2e9b`): el APK de la variante `supabase` compila con `EXPO_PUBLIC_REQUIRE_ACCOUNT=false` y el run [35523944800](https://github.com/thejowe/lockin/actions/runs/35523944800) sale verde en las dos variantes al primer intento. El precio era que el registro no lo recorría nadie en un dispositivo — **pagado el 2026-09-22**: la tercera variante `registro` de la matriz de `e2e.yml` (`register.yaml` + `register-confirm.yaml` + `mail.mjs`, correo por Mailpit y vuelta por `lockin://auth/callback`) recorre el alta entera y el run [35686717809](https://github.com/thejowe/lockin/actions/runs/35686717809) (`0dd7388`) la deja en `pass` con `registration: verified`, tras pillar de paso dos bugs reales (`cd33cac` y `0dd7388`). Entrar con «Ya tengo cuenta» y «He olvidado mi contraseña» **también se recorren desde el 2026-09-23** (`e2e/sign-in.yaml` y `e2e/password-reset.yaml` encadenados en la variante `registro`; run [35901538450](https://github.com/thejowe/lockin/actions/runs/35901538450), `3d0ed0b`, verde al primer intento). Lo único de esa pantalla que sigue sin recorrerse en un dispositivo es la rama de abandono del perfil irrecuperable, que por construcción no cabe en un flujo que arranca con `clearState: true`. Detalle y evidencia en `todo/calidad.md` (sección del 2026-09-23). Detalle en `todo/perfil.md`.
...
puede correr a la vez que `D2`) · **Ola 4** = D5, sola. Por eso solo la Ola 1
lleva etiqueta de herramienta: las demás están bloqueadas por la ola anterior,
no sin decidir.

Son **ocho** hallazgos, no siete. Los siete primeros son los de la auditoría del
2026-09-17; el octavo salió el 2026-09-19 cronometrando el rojo intermitente del
job «Contrato Supabase» y es el mismo defecto de `postgres_changes`, pero visto
desde producción en vez de desde un test.
...
- [x] Hallazgo 8 — al reconectar se pierde en silencio lo ocurrido sin red (orden `D5`, `datos`, Ola 4). **Hecho el 2026-09-19.** Los tres canales del backend de Supabase son `postgres_changes`, que no reemite: tras una caída de red el `phx_join` nuevo llega vacío y la pantalla se queda con el dato viejo hasta el siguiente cambio, que puede no llegar nunca. `subscribeResyncingOnRejoin()` (`src/data/supabase/realtime.ts`) suscribe el canal y avisa **solo a partir del segundo `SUBSCRIBED`**, que es siempre un reenganche; la pantalla relee, que es todo lo que hace falta porque los repositorios no guardan estado. No se avisa en el primer `join`: quien se acaba de suscribir ya ha leído, y hacerlo rompe la cuenta exacta de `messages › avisa solo a los suscriptores de ese hilo`, que es lo que demuestra que `emittedLocally` funciona (esa vía se intentó y se revirtió entera). El contrato lo prueba con una caída de red de verdad: `ContractBackend` gana `dropRealtime()`/`restoreRealtime()` **opcionales** —el mock no tiene conexión que cortar y el caso se salta ahí, como con `canTimeTravel`— y el backend de Supabase los implementa con `realtime.disconnect()`/`connect()`. Detalle en `docs/plan/todo/datos.md`
