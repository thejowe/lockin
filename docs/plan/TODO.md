...
- [x] **[Claude]** «Ya tengo cuenta» en el onboarding (`perfil`): login opcional con `signInWithEmail`/`sendPasswordReset` para recuperar la cuenta en otro dispositivo. Cerrado el 2026-09-19: `npm run typecheck`, `npm run lint` y `npm test -- --ci --runInBand` en verde, cobertura sobre el suelo. **No verificado:** el flujo real contra Supabase en un dispositivo. Detalle y evidencia en `todo/perfil.md`.
- [x] **[Claude]** Interfaz de registro con email y contraseña en el onboarding (`perfil`). Cerrado el 2026-09-20: **registro obligatorio** (decisión del usuario; revoca «sin login obligatorio» solo para el alta), y se **asciende la sesión anónima** (`linkEmailToCurrentUser` + `setAccountPassword` tras confirmar el correo), no `signUp`. `npm run typecheck`, `npm run lint` y `npm test -- --ci --runInBand` en verde, cobertura sobre el suelo. **No verificado:** el flujo real contra Supabase (correo de confirmación, cuenta recuperable, entrar desde otro dispositivo). **Cerrado por `calidad` el 2026-09-20** (`35b2e9b`): el APK de la variante `supabase` compila con `EXPO_PUBLIC_REQUIRE_ACCOUNT=false` y el run [35523944800](https://github.com/thejowe/lockin/actions/runs/35523944800) sale verde en las dos variantes al primer intento. El precio era que el registro no lo recorría nadie en un dispositivo — **pagado el 2026-09-22**: la tercera variante `registro` de la matriz de `e2e.yml` (`register.yaml` + `register-confirm.yaml` + `mail.mjs`, correo por Mailpit y vuelta por `lockin://auth/callback`) recorre el alta entera y el run [35686717809](https://github.com/thejowe/lockin/actions/runs/35686717809) (`0dd7388`) la deja en `pass` con `registration: verified`, tras pillar de paso dos bugs reales (`cd33cac` y `0dd7388`). Sigue sin recorrerse en dispositivo entrar con «Ya tengo cuenta» ni «He olvidado mi contraseña» — en curso en Codex. Detalle y evidencia en `todo/calidad.md` (sección del 2026-09-23). Detalle en `todo/perfil.md`.
