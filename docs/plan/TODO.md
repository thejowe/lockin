...
- [ ] **[comprobador]** Confirmar cierre y reapertura en el emulador (antes: «en el Expo Go del usuario»): APK con credenciales de Supabase (`preview` de EAS o build local con `EXPO_PUBLIC_SUPABASE_*`), perfil creado, `am force-stop` + relanzar, y el mismo perfil tiene que seguir ahí. **Sospecha del 2026-09-19:** sin credenciales la app corre contra el mock en memoria y pierde todo al cerrar — la evidencia debe decir contra qué backend corrió (`[lockin] backend de datos:` en logcat).
