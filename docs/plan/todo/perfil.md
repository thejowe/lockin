...
**Un interruptor que hay que conocer: `EXPO_PUBLIC_REQUIRE_ACCOUNT=false`.** Apaga la puerta en tiempo de compilación. Existe porque el E2E de la variante `supabase` (`e2e/full-journey.yaml`) entra anónimo, elige modo y crea el perfil, y **no tiene buzón donde pinchar el enlace**: con la puerta encendida se quedaría en «Crea tu cuenta» y el job caería en rojo. `e2e/` y `.github/` son de `calidad` y no se tocaron aquí. **Cerrado por `calidad` el 2026-09-20** (`35b2e9b`): `buildEnv()` en `e2e/run.mjs` pone la variable en esa build desechable y el run [35523944800](https://github.com/thejowe/lockin/actions/runs/35523944800) vuelve a verde. Una build de usuario no lo lleva. Contrapartida, que es de este bloque: la pantalla de registro, el ascenso de la sesión anónima y el enlace del correo **no los recorre nadie en un dispositivo** — siguen cubiertos solo por Jest con dobles, y por eso el punto sin marcar de «Lo que NO se ha verificado» sigue sin marcar.
