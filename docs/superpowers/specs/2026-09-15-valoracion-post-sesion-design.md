...
| E2E Android | Flujo nuevo `e2e/session-rate.yaml` tras `session.yaml`. **No lleva fixture de seed propio**: otro trigger dejaría dos sesiones en el match y la viva gana a la valorable, así que la tarjeta nunca llegaría a `valorar`. En su lugar, `e2e/run.mjs` envejece entre flujos la sesión que el recorrido acaba de vivir y añade la fila de asistencia de la otra persona; el flujo toca `Genial` y `e2e/verify.mjs` comprueba la fila en `session_ratings` |
